...
downsample = lambda img, rate=RATE: cv2.pyrDown(img, np.array(img[::int(1 / rate), ::int(1 / rate), :], dtype=np.uint8),
                                                (int(img.shape[1] * rate), int(img.shape[0] * rate)))
upsample = lambda img, rate=int(1 / RATE): cv2.pyrUp(img,
                                                     np.zeros((img.shape[1] * 2, img.shape[0] * 2, 3), dtype=np.uint8),
